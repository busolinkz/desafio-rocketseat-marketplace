...
export class ProductsService {
  ...
  saveProduct(product: NewProductRequest): Observable<NewProductResponse> {
    return this._httpClient.post<NewProductResponse>('http://localhost:3000/api/products', product);
  }

  getProducts(): Observable<ProductsResponse> {
    return this._httpClient.get<ProductsResponse>('http://localhost:3000/api/products').pipe(shareReplay(1));
  }
}
